...
// Keyboard shortcut for auto-clicker (000)
document.addEventListener('keydown', function(event) {
    // Check if we're typing numbers
    if (event.key >= '0' && event.key <= '9') {
        // Simple approach: track last 3 keys pressed
        if (!window.cheatKeys) window.cheatKeys = [];
        window.cheatKeys.push(event.key);
        if (window.cheatKeys.length > 3) window.cheatKeys.shift();
        
        // Check if last 3 keys are 000
        if (window.cheatKeys.join('') === '000') {
            event.preventDefault();
            toggleAutoClicker();
            // Clear the cheat keys after activation
            window.cheatKeys = [];
        }
    }
});
...
function toggleAutoClicker() {
    if (autoClickerActive) {
        ...
        // Click every 0.00000000001 seconds (10 nanoseconds)
        // Note: JavaScript has minimum timeout of ~1ms, so we'll simulate this
        // by making multiple clicks per millisecond
        autoClickerInterval = setInterval(function() {
            // Make 1000 clicks per millisecond to simulate 0.00000000001 second intervals
